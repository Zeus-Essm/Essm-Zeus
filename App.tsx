...
const App: React.FC = () => {
    ...
    const [posts, setPosts] = useState<Post[]>([]); // Começa vazio para usuários reais
    ...
    const fetchUserData = async (userId: string) => {
        const { data: freshProfile } = await supabase.from('profiles').select('*').eq('user_id', userId).single();
        if (freshProfile) {
            ...
            setFolders(f.data || []);
            setProducts(p.data || []);
            // Nota: Em um app real, o feed 'posts' viria filtrado ou global
            
            if (freshProfile.account_type === 'business') {
                setBusinessProfile({
                    id: userId,
            ...
            } else if (freshProfile.account_type === 'personal') {
    ...
    const handleSelectAccountType = async (type: 'personal' | 'business') => {
        ...
        try {
            ...
            toast.success(`Conta ${type === 'business' ? 'Empresarial' : 'Pessoal'} configurada!`);
        } catch (err: any) {
    ...
    const renderScreen = () => {
        ...
        if (viewedProfileId && viewedProfileData) {
            ...
            return (
                <VendorDashboard 
                    ...
                    onCreateFolder={async (t) => {}} onCreateProductInFolder={async (f, d) => {}} onMoveProductToFolder={async (p, f) => {}}
                    onUpdateProfile={() => {}} onUpdateProfileImage={() => {}} onNavigateToProducts={() => {}}
                    onLikePost={(id) => {}} onAddComment={() => {}} onItemClick={startTryOn} onViewProfile={setViewedProfileId}
        ...
        switch (currentScreen) {
            case Screen.Login: 
                return <LoginScreen onNavigateToSignUp={() => setCurrentScreen(Screen.AccountTypeSelection)} />;
            
            case Screen.AccountTypeSelection: 
                return <AccountTypeSelectionScreen onSelect={handleSelectAccountType} />;
            ...
            case Screen.VendorDashboard: 
                return businessProfile && profile && (
                    <VendorDashboard 
                        ...
                        onCreateFolder={async (t) => {}} onCreateProductInFolder={async (f, d) => {}} onMoveProductToFolder={async (p, f) => {}} 
                        onUpdateProfile={() => {}} onUpdateProfileImage={() => {}} onNavigateToProducts={() => setCurrentScreen(Screen.VendorProducts)} 
                        onLikePost={(id) => {}} onAddComment={() => {}} onItemClick={startTryOn} onViewProfile={setViewedProfileId} 
                    />
                );

            case Screen.Feed: 
                return profile && <FeedScreen posts={posts} stories={[]} profile={profile} businessProfile={businessProfile} isProfilePromoted={false} promotedItems={[]} onBack={() => {}} onItemClick={startTryOn} onAddToCartMultiple={() => {}} onBuyMultiple={() => {}} onViewProfile={setViewedProfileId} onSelectCategory={(cat) => setViewedProfileId(cat.id)} onLikePost={(id) => {}} onAddComment={() => {}} onNavigateToAllHighlights={() => {}} onStartCreate={() => { setCurrentScreen(Screen.ImageSourceSelection); }} unreadNotificationCount={unreadCount} onNotificationsClick={() => setIsNotificationsOpen(true)} onSearchClick={() => setCurrentScreen(Screen.Search)} />;
            
            case Screen.Home: 
                return profile && <HomeScreen loggedInProfile={profile} viewedProfileId={null} realBusinesses={realBusinesses} onUpdateProfile={() => {}} onUpdateProfileImage={() => {}} onSelectCategory={(cat) => setViewedProfileId(cat.id)} onNavigateToFeed={() => setCurrentScreen(Screen.Feed)} onNavigateToMyLooks={() => {}} onNavigateToCart={() => setCurrentScreen(Screen.Cart)} onNavigateToChat={() => {}} onNavigateToRewards={() => {}} onStartTryOn={() => { setCurrentScreen(Screen.ImageSourceSelection); }} isCartAnimating={false} onBack={() => {}} posts={posts} onItemClick={startTryOn} onViewProfile={setViewedProfileId} onNavigateToSettings={() => setIsSettingsOpen(true)} onSignOut={() => supabase.auth.signOut()} unreadNotificationCount={unreadCount} unreadMessagesCount={0} onOpenNotificationsPanel={() => setIsNotificationsOpen(true)} isFollowing={false} onToggleFollow={() => {}} followersCount={0} followingCount={0} onLikePost={(id) => {}} onAddComment={() => {}} onSearchClick={() => setCurrentScreen(Screen.Search)} />;
            
            case Screen.ImageSourceSelection: 
                return <ImageSourceSelectionScreen onImageUpload={handlePhotoObtained} onUseCamera={() => setCurrentScreen(Screen.Camera)} onBack={() => setCurrentScreen(Screen.Feed)} />;
            
            case Screen.Camera: 
                return <CameraScreen onPhotoTaken={handlePhotoObtained} onBack={() => setCurrentScreen(Screen.ImageSourceSelection)} />;
            
            case Screen.Generating: 
                return userImage && <LoadingIndicator userImage={generatedImage || userImage} />;
            
            case Screen.Result: 
                return generatedImage && <ResultScreen generatedImage={generatedImage} items={vtoItems} categoryItems={[]} onBuy={() => {}} onUndo={() => { setCurrentScreen(Screen.Feed); }} onStartPublishing={() => setShowCaptionModal(true)} onSaveImage={() => {}} onItemSelect={startTryOn} onAddMoreItems={() => setCurrentScreen(Screen.Feed)} onGenerateVideo={() => {}} />;
