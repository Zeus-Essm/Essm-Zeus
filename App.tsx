    ...
    useEffect(() => {
        const handleAuthState = async (currentSession: Session | null) => {
            ...
            if (currentSession?.user) {
                ...
                // 2. Recarregar Dados Reais (Reidratação SSOT)
    ...
    const renderScreenContent = () => {
        ...
        switch (currentScreen) {
            ...
            case Screen.VendorProducts:
                return businessProfile && <VendorProductsScreen 
            ...
            case Screen.Feed:
                return profile && <FeedScreen 
                    posts={posts} stories={INITIAL_STORIES} profile={profile} businessProfile={businessProfile} isProfilePromoted={false} promotedItems={[]}
                    ...
                    onViewProfile={setViewedProfileId} onSelectCategory={c => { setCurrentScreen(Screen.SubCategorySelection); }}
                    ...
                    unreadNotificationCount={0} onNotificationsClick={() => {}} onSearchClick={() => setCurrentScreen(Screen.Search)}
                />;
            case Screen.Home:
                return profile && <HomeScreen 
                    ...
                    isCartAnimating={false} onBack={() => setViewedProfileId(null)} posts={posts} onItemClick={setRecommendationItem}
                    onViewProfile={(id) => { setViewedProfileId(id); setCurrentScreen(Screen.Home); }} onNavigateToSettings={() => setCurrentScreen(Screen.Settings)} onSignOut={handleSignOut}
                    unreadNotificationCount={0} unreadMessagesCount={0} onOpenNotificationsPanel={() => {}}
                    isFollowing={false} onToggleFollow={() => {}}
