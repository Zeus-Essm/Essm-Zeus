  ...
  return (
    <div className="w-full h-full flex flex-col text-white animate-fadeIn bg-black">
      ...
      <main className="flex-grow overflow-y-auto pt-20 p-4">
        <h2 className="text-2xl font-bold mb-4 text-gray-300">Escolha uma coleção</h2>
        <div className="grid grid-cols-2 gap-4 pb-4">
            {(node.subCategories || []).map((subCategory) => (
              <div
                key={subCategory.id}
                onClick={() => onSelectSubCategory(subCategory)}
                className="relative w-full h-48 rounded-2xl overflow-hidden cursor-pointer group transform hover:scale-105 transition-transform duration-300"
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-4">
                  <h2 className="text-2xl font-black tracking-tighter uppercase">{subCategory.name}</h2>
                </div>
