...
import { ArrowLeftIcon, ShoppingBagIcon, EyeIcon, UsersIcon, StarIcon } from './IconComponents';
...
const VendorAnalyticsScreen: React.FC<VendorAnalyticsScreenProps> = ({ onBack }) => {
  return (
    <div className="w-full h-full bg-white flex flex-col animate-fadeIn font-sans">
      {/* Header */}
      <div className="px-4 pt-4 pb-2 flex items-center gap-4 bg-white border-b border-zinc-50">
        <button 
          onClick={onBack} 
          className="p-2 bg-zinc-50 rounded-xl text-zinc-500 hover:bg-zinc-100 transition-colors"
        >
          <ArrowLeftIcon className="w-5 h-5" />
        </button>
        <h1 className="text-lg font-black uppercase italic text-zinc-900 tracking-tighter">
          VISÃO GERAL
        </h1>
      </div>
            ...
            <div className="flex items-end justify-between h-24 gap-3">
                {['SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SÁB', 'DOM'].map((day) => (
                    <div key={day} className="flex-1 flex flex-col items-center gap-3">
                        <div className="w-full bg-zinc-50 rounded-t-lg h-1.5 border border-zinc-100/50"></div>
